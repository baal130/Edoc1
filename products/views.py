...
class ProductDetailSlugView(ObjectViewedMixin, DetailView):
	...
	def get_context_data(self, *args, **kwargs):
		...
		product_instance=context.get('object')
		
	
		try:
			remark_list=ProductRemark.objects.filter(product__in=qs)
		except:
			remark_list=None
		context['remark_list']=remark_list
		try:
			billing_profile_instance,new_billing=BillingProfile.objects.new_or_get(self.request)
			product_purchases=product_instance.productpurchase_set.filter(billing_profile=billing_profile_instance)
			print( product_purchases)
			if(product_purchases):
				context['user_has_access']=True
			else:
